...
def extract_words(text):
    ...
    ['cat', 'on', 'my', 'keyboard']
    """
    # Assume word is a sequence of only ASCII letters
    # string ascii_letters contains all letters in ASCII character set
...
def analyze_tweet_sentiment(tweet):
    ...
    for w in words:
        if has_sentiment(get_word_sentiment(w)):
            ...
            sentiment_value_list.append(get_word_sentiment(w))


    if sentiment_value_list:
        return make_sentiment(total/len(sentiment_value_list))
...
def find_centroid(polygon):
    ...
    (1.0, 2.0, 0.0)
    """
    # latitudes = x values, longitudes = y values
    # polygon is a list
    # polygon[i] accesses the element at i
    ...
    def area():
        total = 0.0
        for i in range(num_vertices): # i = index of polygon list
            # Shoelace Formula: find area given vertices of polygon
            # Cross product of vectors from current vertex to next vertex
            total += ( x(p[i]) * y(p[i+1]) ) - ( x(p[i+1]) * y(p[i]) )
        return total / 2.0

    area = area()

    if area == 0: # return x and y of first position as centroid
...
def find_state_center(polygons):
    ...
    # Given a list of polygons, we have to total the centroids lat and lon values
    # and then divide it by the area

    # polygons is a list of polygons: [p1, p2, p3]
    total_x = 0
    total_y = 0
    total_area = 0
    for p in polygons:
        p_values = find_centroid(p)
        total_x += p_values[0] * p_values[2]
        total_y += p_values[1] * p_values[2]
        total_area += p_values[2]
...
def group_tweets_by_state(tweets):
    ...
    # us_states is dictionary with {state, polygon}
    # Create dictionary with {state, positions (object)} by using find_state_center
    # which returns position object
    def create_states_position_dict(us_states):
        states_position_dict = {}
        for state in us_states:
            # access the value correlated to the state
            states_position_dict[state] = find_state_center(us_states[state])
        return states_position_dict
    
    #Calculate the state which is closest to the tweet location
    #geo_distance is method to find distance between 2 positions
    #returns state postal code
    def get_closest_state(states_position_dict, tweet):
        tweet_position_object = tweet_location(tweet)
        #initially, set closest_state to just first value. It will be overridden.
        closest_state = list(states_position_dict.keys())[0]
        distance = geo_distance(states_position_dict[closest_state], tweet_position_object)

        for state in states_position_dict:
            if distance > geo_distance(states_position_dict[state], tweet_position_object):
                distance = geo_distance(states_position_dict[state], tweet_position_object)
                closest_state = state
        return closest_state

    # Create dictionary with {state postal code, position}
    states_position_dict = create_states_position_dict(us_states)

    #Finally, create dictionary with {state, list of tweets}
    tweets_by_state = {}
    for tweet in tweets:
        state = get_closest_state(states_position_dict, tweet)
        if state in tweets_by_state: # if it already exists, then append
            tweets_by_state[state].append(tweet)
        else: # doesn't exist, so make a new list
...
def average_sentiments(tweets_by_state):
    ...
    # tweets_by_state is dictionary { STATE, list of tweets}
    averaged_state_sentiments = {}

    for state in tweets_by_state.keys():
        tweet_list_of_state = tweets_by_state[state]
        total_state_sentiment_value = 0
        num_tweets_with_sentiment = 0
        for tweet in tweet_list_of_state:
            tweet_has_sentiment = has_sentiment(analyze_tweet_sentiment(tweet))
            if tweet_has_sentiment:
                tweet_sentiment_value = sentiment_value(analyze_tweet_sentiment(tweet))
                total_state_sentiment_value += tweet_sentiment_value
                num_tweets_with_sentiment += 1
        if num_tweets_with_sentiment != 0:
            averaged_state_sentiments[state] = (total_state_sentiment_value / num_tweets_with_sentiment)
